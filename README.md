...
**A minimal, opinionated agentic coding workflow for Claude Code. Two commands. Thirteen files. No framework.**
...
<sub>Claude Code's `/verify` builds and drives your running app. This kit's `scripts/verify.sh` is the static gate — lint, types, tests, build. They are complementary, not duplicates.</sub>

**Custom commands merged into skills.** And a 200-agent marketplace is not capability: every agent's description sits in your context window on every session, and Claude Code warns you at startup once they total 15,000 tokens.
